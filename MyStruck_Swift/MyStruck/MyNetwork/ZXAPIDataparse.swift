        ...
        if let objB = objA as? Dictionary<String,Any> {
            ...
            switch status {
                case ZXAPI_LOGIN_INVALID:
                    if let url = url, !MyNetwork.notPostLoginInvalid(url: url) {
...
extension MyNetwork {
    ...
    fileprivate static func notCheckLogin(url:String) -> Bool {
        if  url == ZXAPI.api(address: ZXAPIConst.User.getDictList) ||
            url == ZXAPI.api(address: ZXAPIConst.User.verCode) ||
            url == ZXAPI.api(address: ZXAPIConst.User.telLogin) ||
            url == ZXAPI.api(address: ZXAPIConst.User.WXLogin) ||
            url == ZXAPI.api(address: ZXAPIConst.User.register) ||
            url == ZXAPI.api(address: ZXAPIConst.User.verInviteCode) ||
            url == ZXAPI.api(address: ZXAPIConst.System.time) {
//            url == ZXAPI.api(address: ZXAPIConst.Store.labelList) ||
//                url == ZXAPI.api(address: ZXAPIConst.Store.goodsList) ||
//                url == ZXAPI.api(address: ZXAPIConst.Store.goodsDetail) ||
//                url == ZXAPI.api(address: ZXAPIConst.Home.behavior)
            return true
        }
        return false
    }
...
//        if  url == ZXAPI.api(address: ZXAPIConst.Store.goodsList) ||
//            url == ZXAPI.api(address: ZXAPIConst.Store.goodsDetail) ||
    ...
    @discardableResult class func asyncRequest(withUrl url:String,
        ...
        let task = MyNetwork.my_asyncRequest(withUrl: url, params: tempP, method: method, completion: { (objA, stringValue) in
    ...
    @discardableResult class func uploadImage(to url:String!,
        ...
        let task = MyNetwork.my_uploadImage(to: url, images: images, params: tempP, completion: { (objA, stringValue) in
    ...
    @discardableResult class func fileupload(to url:String!,
        ...
        let task = MyNetwork.my_fileupload(to: url, name: name, fileName: fileName, mimeType: mimeType, fileData: fileData, params: tempP, completion: { (objA, stringValue) in
            ZXAPIDataparse.parseJsonObject(objA, success: { (code, dic) in
                completion(true, code, dic, stringValue!, nil)
            }, offline: { (code, error, dic) in
                completion(false, code, dic, stringValue!, error)
